...
const ExperienceSection = ({ userData, isOwnProfile, onSave }) => {
  ...
  const handleDeleteExperience = (id, index) => {
    setExperiences((prev) =>
      id ? prev.filter((exp) => exp._id !== id) : prev.filter((_, i) => i !== index)
    );
  };
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6 mb-6">
      ...
      {experiences.length === 0 ? (
        <p className="text-gray-500 mb-4">No experience yet.</p>
      ) : (
        experiences.map((exp, index) => (
          <div
            key={exp._id || `new-exp-${index}`}
            ...
            <div className="flex items-start">
              ...
              <div>
                ...
                <p className="text-gray-500 text-sm">
                  ...
                  {exp.currentlyWorking || !exp.endDate ? "Present" : formatDate(exp.endDate)}
                </p>
