...
const EducationSection = ({ userData, isOwnProfile, onSave }) => {
  ...
  const handleDeleteEducation = (id, index) => {
    setEducations((prev) =>
      id ? prev.filter((edu) => edu._id !== id) : prev.filter((_, i) => i !== index)
    );
  };
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6 mb-6">
      ...
      {educations.length === 0 ? (
        <p className="text-gray-500 mb-4">No education yet.</p>
      ) : (
        educations.map((edu, index) => (
          <div key={edu._id || `new-${index}`} className="mb-4 flex justify-between items-start">
